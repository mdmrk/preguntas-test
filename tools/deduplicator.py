...
class TestDeduplicator:
    ...
    def questions_are_duplicate(q1: Question, q2: Question) -> bool:
        ...
        if TestDeduplicator.normalize_text(
            q1.question
        ) != TestDeduplicator.normalize_text(q2.question):
            return False

        # Compare option sets (order doesn't matter)
        options1_normalized = {
            TestDeduplicator.normalize_text(opt) for opt in q1.options
        }
        options2_normalized = {
            TestDeduplicator.normalize_text(opt) for opt in q2.options
        }

        return options1_normalized == options2_normalized

    @staticmethod
    def standardize_option_order(
        canonical_question: Question, duplicate_question: Question
    ) -> Question:
        """
        Reorder the options of duplicate_question to match the order in canonical_question.
        Also updates the answer accordingly.
        """
        # Create mapping from normalized option text to original option text for both questions
        canonical_options_normalized = [
            TestDeduplicator.normalize_text(opt) for opt in canonical_question.options
        ]
        duplicate_options_normalized = [
            TestDeduplicator.normalize_text(opt) for opt in duplicate_question.options
        ]
        ...
        for i, canonical_opt_norm in enumerate(canonical_options_normalized):
            ...
            for j, duplicate_opt_norm in enumerate(duplicate_options_normalized):
                if canonical_opt_norm == duplicate_opt_norm:
                    ...
                    if j == duplicate_question.answer:
        ...
        return Question(
            ...
            answer=new_correct_answer_index,
    ...
    @staticmethod
    def merge_tags(
        tags1: Optional[List[str]], tags2: Optional[List[str]]
    ) -> Optional[List[str]]:
        """
        ...
        for idx, leader in duplicate_groups.items():
            ...
            if idx != leader:
                ...
                tags_merges[leader] = TestDeduplicator.merge_tags(
                    tags_merges[leader], questions[idx].tags
                )
...
def process_file(file_path: Path) -> Tuple[int, int]:
    ...
    try:
        ...
        if num_removed > 0:
            ...
            print(
                f"  ✅ Merged {num_removed} duplicate(s): {original_count} → {final_count}"
            )
        else:
